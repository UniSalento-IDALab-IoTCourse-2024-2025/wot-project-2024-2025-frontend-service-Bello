            ...
            {trips.map((trip) => {
              ...
              return (
                <React.Fragment key={trip.id}>
                  {/* Trip Row (two levels) */}
                  <div className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                    ...
                    <div className="px-5 pt-4 pb-2 flex items-start gap-3">
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        ...
                        <p className="text-sm font-medium text-gray-900 dark:text-white leading-snug">
                          {trip.departureAddress || `${trip.departureLatLng?.lat.toFixed(3)}, ${trip.departureLatLng?.lng.toFixed(3)}`}
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        ...
                        <p className="text-sm font-medium text-gray-900 dark:text-white leading-snug">
                          {trip.arrivalAddress || `${trip.arrivalLatLng?.lat.toFixed(3)}, ${trip.arrivalLatLng?.lng.toFixed(3)}`}
                        </p>
                      </div>
                    </div>

                    {/* Level 2: Details + Actions */}
                    <div className="px-5 pb-4 pt-1 flex flex-wrap items-center gap-x-5 gap-y-2">
                      <div className="flex items-center gap-2">
                        <svg className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 0 0-10.026 0 1.106 1.106 0 0 0-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12" />
                        </svg>
                        <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">{trip.vehicleName}</span>
                        {trip.refrigerated && (
                          <span className="px-2 py-0.5 text-[10px] font-medium rounded-md bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-300">Refrigerated</span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-sm text-gray-600 dark:text-gray-400">{formatDate(trip.arrivalDate)}</span>
                      </div>
                      <div className="w-px h-4 bg-gray-300 dark:bg-gray-600" />
                      <span className="text-sm text-gray-600 dark:text-gray-400">{trip.distanceKm?.toFixed(1) || '0'} km</span>
                      <div className="flex-1" />
                      <div className="flex items-center gap-1">
                        <button onClick={() => handleToggleShipments(trip)} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors">
                          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m20.25 7.5-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z" /></svg>
                          Shipments
                        </button>
                        <button onClick={() => handleShowMap(trip)} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-950/20 rounded-lg transition-colors">
                          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z" /></svg>
                          Map
                        </button>
                        <button onClick={() => handleDeleteClick(trip)} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-lg transition-colors">
                  ...
                  {/* Shipments Expanded */}
                  {isExpanded && (
                    <div className="bg-gray-50 dark:bg-gray-800/20 border-t-2 border-gray-300 dark:border-gray-600 px-5 py-5">
                      ...
                      {loadingShipments ? (
                        ...
                        <div className="space-y-3">
                          {shipments.map((shipment) => (
                            <div key={shipment.id} className="bg-white dark:bg-gray-900 rounded-xl border-2 border-gray-400 dark:border-gray-600 p-4">
                              <div className="flex items-center gap-2 mb-3">
                                <div className="flex items-center gap-1.5 min-w-0 flex-1">
                                  <div className="w-2 h-2 rounded-full bg-green-500 flex-shrink-0" />
                                  <span className="text-sm text-gray-700 dark:text-gray-300">{shipment.departureAddress}</span>
                                </div>
                                ...
                                <div className="flex items-center gap-1.5 min-w-0 flex-1">
                                  ...
                                  <span className="text-sm text-gray-700 dark:text-gray-300">{shipment.arrivalAddress}</span>
                                </div>
                              </div>
                              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400">
                                <span>{shipment.width}&times;{shipment.height}&times;{shipment.length} cm</span>
                                <span>{shipment.weight} kg</span>
                                <span className="font-semibold text-primary-600 dark:text-primary-400">&euro;{shipment.price?.toFixed(2) || '0.00'}</span>
                                <div className="flex-1" />
                                <button onClick={() => handleShowShipmentMap(shipment)} className="text-xs text-gray-400 dark:text-gray-500 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
                                  View on map &rarr;
                                </button>
                              </div>
                            </div>
